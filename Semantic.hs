module Semantic where
  ...
  eval expr =
    case expr of
      ...
      Add (I n) (I m) -> I (n + m)
      Add (I n) e -> let e' = eval e in Add (I n) e'
      Add e1 e2 -> let e1' = eval e1 in Add e1' e2
      Mul (I n) (I m) -> I (n * m)
      Mul (I n) e -> let e' = eval e in Mul (I n) e'
      Mul e1 e2 -> let e1' = eval e1 in Mul e1' e2
      Succ (I n) -> I (n + 1)
      Succ e -> Succ (eval e)
      ...
      Not e -> Not (eval e)
      And (B p) (B q) -> B (p && q)
      And (B p) e -> let e' = eval e in And (B p) e'
      And e1 e2 -> let e1' = eval e1 in And e1' e2
      Or (B p) (B q) -> B (p || q)
      Or (B p) e -> let e' = eval e in Or (B p) e'
      Or e1 e2 -> let e1' = eval e1 in Or e1' e2
      Lt (I n) (I m) -> B (n < m)
      Lt (I n) e -> let e' = eval e in Lt (I n) e'
      Lt e1 e2 -> let e1' = eval e1 in Lt e1' e2
      Gt (I n) (I m) -> B (n > m)
      Gt (I n) e -> let e' = eval e in Gt (I n) e'
      Gt e1 e2 -> let e1' = eval e1 in Gt e1' e2
      Eq (I n) (I m) -> B (n == m)
      Eq (I n) e -> let e' = eval e in Eq (I n) e'
      Eq e1 e2 -> let e1' = eval e1 in Eq e1' e2
      If (B q) e1 e2 -> if q then e1 else e2
      If e1 e2 e3 -> If (eval e1) e2 e3
      Let i (I n) e2 -> subst e2 (i, (I n))
      Let i (B p) e2 -> subst e2 (i, (B p))
      Let i e1 e2 -> Let i (eval e1) e2
